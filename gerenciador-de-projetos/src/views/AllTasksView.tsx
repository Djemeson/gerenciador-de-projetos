...
export function AllTasksView() {
  const { tasks, projects, selectedTaskId, openNewTask } = useAppStore()
  ...
  // Stats
  const total   = filtered.filter(t => t.status !== 'done').length
  const urgent  = filtered.filter(t => t.priority === 'urgent' && t.status !== 'done').length
  const overdue = filtered.filter(t => t.dueDate && t.status !== 'done' && new Date(t.dueDate) < new Date()).length
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <TaskList tasks={filtered} showProject sortBy={sortBy} />
      </div>
