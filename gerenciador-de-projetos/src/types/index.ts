...
export type View       = 'my_tasks' | 'all_tasks' | 'projects' | 'project_detail' | 'calendar' | 'reports' | 'inbox'
...
// Bloco de conteúdo — texto, imagem ou áudio, inseridos onde o usuário quiser
export type ContentBlockType = 'text' | 'image' | 'audio'
export interface ContentBlock {
  id: string
  type: ContentBlockType
  text?: string      // para blocos de texto
  data?: string      // base64 para imagem/áudio
  name?: string
  mimeType?: string
}
...
export interface Task {
  ...
  title: string
  description: string
  blocks: ContentBlock[]     // conteúdo misto: texto + imagens + áudios
  status: TaskStatus
  priority: Priority
  dueDate: string | null
  assignee: string
...
// Migra tarefas antigas (com content/attachments) para o formato de blocos
