...
export function TagInput({ value, onChange }: TagInputProps) {
  ...
  return (
    <div className="tag-input-root relative">
      <div
        className="flex flex-wrap gap-1 min-h-[34px] px-2 py-1 border border-gray-200 rounded-lg bg-white cursor-text"
        onClick={() => { inputRef.current?.focus(); setOpen(true) }}
      >
        {value.map(tag => (
          <span key={tag} className="inline-flex items-center gap-1 text-xs px-2 py-0.5 bg-brand-50 text-brand-700 rounded-full border border-brand-200">
            {tag}
