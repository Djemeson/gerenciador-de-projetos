...
export function Sidebar() {
  ...
  // Projetos ordenados por GUT score (maior = mais prioritário)
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-4 space-y-0.5 scrollbar-none">
        ...
        <div>
          <div className="flex items-center justify-between px-2.5 py-1.5">
            ...
            <button onClick={openNewProject} className="w-5 h-5 flex items-center justify-center rounded text-gray-400 hover:bg-gray-200 hover:text-gray-600 transition-colors" title="Novo projeto">
              <Plus size={12} />
            </button>
          </div>
            ...
            {sortedProjects.map(p => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full flex-shrink-0 opacity-80"
                    style={{ background: tier.bg, color: tier.color }}
                  >
                    {p.gut.score}
                  </span>
                  <ChevronRight size={11} className="opacity-0 group-hover:opacity-40 transition-opacity flex-shrink-0" />
                </button>
              )
            })}
          </div>
